<main class="w-full max-w-7xl mx-auto px-6 py-8">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
        ...
        <span class="material-symbols-outlined text-xs">chevron_right</span>
        <span class="font-bold text-slate-900 dark:text-slate-100">{{ $isEdit ? 'Edit Produk' : 'Tambah Produk' }}</span>
    </nav>
    ...
    <div class="bg-white dark:bg-slate-900/50 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div class="p-6 sm:p-8">
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $isEdit ? 'Edit Produk' : 'Tambah Produk Baru' }}</h1>
            ...
            <form wire:submit="saveProduct" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <label for="photo-upload"
                        class="flex flex-col items-center justify-center border-2 border-dashed {{ $photo || ($isEdit && $product && $product->image_path) ? 'border-primary' : 'border-slate-300 dark:border-slate-700' }} rounded-xl bg-slate-50 dark:bg-slate-800/50 p-8 group hover:border-primary transition-colors cursor-pointer relative overflow-hidden h-64"
                    >
                        @if ($photo)
                        ...
                        @elseif ($isEdit && $product && $product->image_path)
                            <div class="absolute inset-0 w-full h-full bg-slate-100 dark:bg-slate-900 overflow-hidden pointer-events-none">
                                <img 
                                    src="{{ Storage::url($product->image_path) }}" 
                                    class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.02]"
                                    alt="Foto Produk Sekarang"
                                >
                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <span class="text-white font-bold flex items-center gap-2">
                                        <span class="material-symbols-outlined">sync</span> Ganti Foto
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select wire:model="category_id" class="w-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all appearance-none dark:text-white text-sm" id="category">
                ...
                <!-- Location Field -->
                <div class="space-y-2">
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300" for="location">Lokasi COD (Contoh: FMIPA, Gedung A)</label>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">location_on</span>
                        <input wire:model="location" class="w-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all placeholder:text-slate-400 dark:text-white text-sm" id="location" placeholder="Masukkan lokasi penyerahan barang COD" type="text"/>
                    </div>
